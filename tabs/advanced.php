...
if (isset($_POST['save-github-updater-settings'])) {
    check_admin_referer('ti-save-github-settings');
    ...
    update_option('trustindex_github_user', $gh_user, false);
    update_option('trustindex_github_repo', $gh_repo, false);
    header('Location: admin.php?page=' . esc_attr($_page) . '&tab=advanced&gh_saved=1');
    exit;
}
...
<div class="ti-box" style="margin-bottom:25px;">
    <div class="ti-box-header">🚀 GitHub Auto-Updater Settings (EmonAhammed1 / Google-Review-Plugin)</div>
    <p>Connected to <code>https://github.com/EmonAhammed1/Google-Review-Plugin</code>. When you publish a release or push a tag on GitHub, WordPress will automatically display a 1-click update notification in your WordPress dashboard!</p>
    
    <?php if (isset($_GET['gh_saved'])): ?>
        <div class="ti-notice ti-notice-success" style="margin-bottom:15px;"><p>GitHub Auto-Updater settings saved successfully!</p></div>
    <?php endif; ?>

    <form method="post" action="">
        <?php wp_nonce_field('ti-save-github-settings'); ?>
        <input type="hidden" name="save-github-updater-settings" value="1" />
        <div style="display:flex; gap:15px; flex-wrap:wrap; margin-bottom:15px;">
            <div style="flex:1; min-width:220px;">
                <label style="font-weight:600; display:block; margin-bottom:5px;">GitHub Username</label>
                <input type="text" name="gh_user" value="<?php echo esc_attr(get_option('trustindex_github_user', 'EmonAhammed1')); ?>" style="width:100%; padding:8px 12px; border:1px solid #cbd5e1; border-radius:6px;" required />
            </div>
            <div style="flex:1; min-width:220px;">
                <label style="font-weight:600; display:block; margin-bottom:5px;">GitHub Repository Name</label>
                <input type="text" name="gh_repo" value="<?php echo esc_attr(get_option('trustindex_github_repo', 'Google-Review-Plugin')); ?>" style="width:100%; padding:8px 12px; border:1px solid #cbd5e1; border-radius:6px;" required />
            </div>
        </div>
        <button type="submit" class="button button-primary" style="background:#10b981; border-color:#10b981; font-weight:600;">Save GitHub Updater Settings</button>
    </form>
</div>
